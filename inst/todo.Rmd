...
## **BUG**: show method for >2 dimensional array.  -- fixed in DelayedArray 0.7.28. 
...
## How about the other data inside VCF? 
...
log(ds+5)
colSums(ds)  ## error
## ds[colSums(ds) > 2, ,drop = FALSE]
```

**BUG**, to be used as example in `VCFArray-methods.R`. 
```{r, type}
fl <- system.file("extdata", "chr22.vcf.gz", package="VariantAnnotation")
