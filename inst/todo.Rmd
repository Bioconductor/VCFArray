...
## **TODO** BUG: in numeric calculation. 
```{r, numeric}
fl <- system.file("extdata", "chr22.vcf.gz", package="VariantAnnotation")
...
## **DOING** generic function `availableVcfEntries`. 

Asked Valerie to define generic function in `VariantAnnotation`. 
availableNames() ==> vcfEntries() or availableVcfEntries()...
...
## **NEW FEATURE TODO**: variant filtering/statistical functions. 
```{r}
...
## **TODO** feature request to support `List` `DelayedArray`. 
...
## **TODO** rename "availableNames" into "vcfFields" as method. 
...
Once merged, will define vcfFields,RangedVcfStack-method (pkg:GenomicFiles), and 
remove the `availableNames` functions from `VCFArray`. 
