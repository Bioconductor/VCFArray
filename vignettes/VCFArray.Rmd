...
The exported function `availableNames()` takes the VCF file path,
`VcfFile` object or `RangedVcfStack` object as input, and returns all
available data entries that could be converted into `VCFArray`
instances. The returned data entry names can be used as input for the
`name` argument in `VCFArray` constructor.
