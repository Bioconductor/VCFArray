...
The exported function `availableNames()` takes the VCF file path or
`VcfFile` object as input, and returns all available data entries that
could be converted into `VCFArray` instances. The returned data entry
names can be used as input for the `name` argumentg in `VCFArray`
constructor.
...
a remote VCF file, the `index` argument must be specified.** Since
